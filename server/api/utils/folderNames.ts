...
export default defineEventHandler(async () => {
  try {
    ...
    try {
      ...
      console.error(
        'Error reading component folder:',
        (fsError as Error).message,
      )
      throw new Error(
        'Failed to read folder directory. Ensure the "components/content" folder exists.',
      )
    }
  } catch (error: unknown) {
    ...
    try {
      ...
      const folders = (JSON.parse(jsonData) as Folder[]).map(
      ...
      return errorHandler({
        success: false,
        message:
          'Failed to fetch folder names from both directory and fallback JSON.',
        statusCode: 500,
      })
    }
  }
