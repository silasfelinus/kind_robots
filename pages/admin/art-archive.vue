      ...
      <template v-else>
        ...
        <section class="kr-panel space-y-3 p-3" aria-label="Batch curation board">
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-2"><span class="badge badge-primary">{{ archive.selectedCount }} selected</span><button type="button" class="kr-btn btn-ghost btn-xs" :disabled="!archive.selectedCount" @click="archive.clearBatchSelection()">Clear</button></div>
          </div>
          <div class="flex flex-wrap gap-2">
            <button v-for="n in 5" :key="`batch-${n}`" type="button" class="kr-btn btn-outline btn-sm" :disabled="!archive.selectedCount || archive.actionPending" @dragover.prevent @drop.prevent="onBatchRating(n)" @click="onBatchRating(n)">{{ n }}★</button>
            <button type="button" class="kr-btn btn-error btn-sm" :disabled="!archive.selectedCount || archive.actionPending" @dragover.prevent @drop.prevent="requestBatchDelete" @click="requestBatchDelete"><Icon name="kind-icon:trash" class="kr-icon-4" /> Delete selected</button>
            <button v-for="preset in archive.presets" :key="preset.id" type="button" class="kr-btn btn-outline btn-sm" :disabled="!archive.selectedCount" :title="`${preset.actionType}: preset execution lands with ArtJob wiring in t-016`" @dragover.prevent @drop.prevent="previewPreset(preset.id)" @click="previewPreset(preset.id)"><Icon name="kind-icon:magic" class="kr-icon-4" /> {{ preset.label }}</button>
          </div>
