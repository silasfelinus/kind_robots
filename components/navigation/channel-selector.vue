...
<template>
  <div
    class="h-full w-full overflow-hidden rounded-2xl border border-base-300 bg-base-100/90 shadow-xl shadow-base-content/10 backdrop-blur"
  >
...
const channelComponentMap: Record<FooterKey, Component> = {
  fx: ButterflyFooter,
  bot: BotFooter,
  art: ArtFooter,
...
function isChannelKey(value: unknown): value is FooterKey {
  return typeof value === 'string' && footerKeys.includes(value as FooterKey)
}

function normalizeChannelKey(value: unknown): FooterKey {
  return isChannelKey(value) ? value : fallbackFooterKey
}

const activeChannel = computed<FooterKey>(() => {
  return normalizeChannelKey(displayStore.footerComponent)
})

const activeComponent = computed<Component>(() => {
  return channelComponentMap[activeChannel.value] ?? ButterflyFooter
})

watchEffect(() => {
  const normalized = normalizeChannelKey(displayStore.footerComponent)
