...
for (const path of [
  ...
  'content/channels/plan/serendipity.md',
  'components/pages/serendipity-voice-page.vue',
]) {
  assert.equal(fs.existsSync(path), false, `obsolete Serendipity identity or placement remains: ${path}`)
}
...
assert.match(channel, /^requiredRole: ADMIN$/m)
