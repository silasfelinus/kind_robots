...
export default defineEventHandler(async (event): Promise<ImageResponse> => {
  try {
    ...
    const selectedGalleryIDs = shuffledIDs.slice(0, Math.min(count, shuffledIDs.length)).map(g => g.id);
    ...
    const selectedImages = allImagesArrays.flatMap((galleryImages) => {
      return galleryImages.length > 0 ? galleryImages[Math.floor(Math.random() * galleryImages.length)] : [];
    });
