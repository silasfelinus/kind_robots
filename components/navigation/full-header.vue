  ...
  >
    <div class="flex h-full w-full min-w-0 items-center gap-2 overflow-hidden px-2 py-0.5">
      <button
        ...
        class="group relative z-20 flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-base-300 bg-base-200 text-base-content shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-secondary hover:bg-secondary hover:text-secondary-content active:translate-y-0 active:scale-95"
      ...
      <div class="flex h-full min-w-0 flex-1 items-center overflow-hidden rounded-2xl border border-base-300 bg-base-200/60 px-2">
        <smart-icons
          :key="smartIconsKey"
...
const headerShellClasses = computed(() => {
  if (isOpen.value) {
    return 'h-20 sm:h-24'
  }
