...
export function errorHandler(error: ErrorHandlerInput): ErrorHandlerOutput {
  ...
  if (error instanceof Error) {
    ...
    if (error.message.includes('Authentication required') || error.message.includes('Invalid API Key')) {
      return {
        success: false,
...
function handlePrismaError(error: Prisma.PrismaClientKnownRequestError): ErrorHandlerOutput {
  switch (error.code) {
    case 'P2002':
      return {
        success: false,
