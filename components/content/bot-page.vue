<template>
  ...
  >
    <div class="w-96 h-96 bg-secondary-300 rounded-full overflow-hidden">
      <p class="mt-4 text-2xl text-black">{{ currentBot.description }}</p>
      <p class="text-2xl text-black">{{ currentBot.userIntro }}</p>
      <h1 class="text-5xl font-semibold mb-4 text-black">{{ currentBot.name }}</h1>
      <div class="w-96 h-96 bg-secondary-300 rounded-full overflow-hidden">
      ...
      <div v-if="channels[currentBot.id]" class="p-4 mt-4 rounded bg-primary text-white">
        <!-- response section -->
        <div v-for="channel in channels[currentBot.id]" :key="channel.id" class="mt-4">
          <!-- chat bubble -->
          <div class="p-4 rounded-lg bg-accent text-white flex items-center space-x-3">
            ...
            <div>
              <h2 class="font-bold">{{ currentBot.name }}</h2>
              <p>{{ channel.message }}</p>
            </div>
          </div>
        </div>
...
let prompt = ref<string | undefined>(currentBot.value?.prompt)
let errorMessage = ref<string>('')
let isLoading = ref<boolean>(false)
let channels = ref<Record<string, Array<{ id: number; message: string }>>>({})

// computed value for bot avatar
let botAvatar = computed(() => currentBot.value?.avatarImage || '/images/avatars/amibot1.png')
    ...
    try {
      const response = await axios.post('/api/botcafe/chat', {
        ...
        messages: [
          {
            role: 'user',
            content: currentBot.value.prompt || 'Hello, world!'
          }
        ],
        temperature: 0.5,
        max_tokens: 1000
      ...
      if (response.status === 200) {
        const botChannels = channels.value[currentBot.value.id] || []
        botChannels.push({ id: Date.now(), message: response.data.message })
        prompt.value = ''
        channels.value = { ...channels.value, [currentBot.value.id]: botChannels }
      } else {
