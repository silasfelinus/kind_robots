...
-- Clear orphaned serverIds before adding constraint
UPDATE `Art` SET `serverId` = NULL WHERE `serverId` IS NOT NULL;
UPDATE `Bot` SET `serverId` = NULL WHERE `serverId` IS NOT NULL;
UPDATE `Chat` SET `serverId` = NULL WHERE `serverId` IS NOT NULL;
-- AddForeignKey
