...
<template>
  <section class="flex min-h-full w-full flex-col gap-4">
    <header class="rounded-2xl border border-base-300 bg-base-200 p-4">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0">
          ...
          <h2 class="mt-1 flex items-center gap-2 text-2xl font-bold text-base-content">
    ...
    <nav class="relative overflow-x-auto rounded-2xl border border-base-300 bg-base-200 p-3">
      <div class="flex min-w-max items-stretch gap-3">
        ...
        >
          <div class="flex w-full items-center justify-between gap-2">
            ...
            <span class="rounded-full bg-base-100/60 px-2 py-0.5 text-[0.65rem] font-black text-base-content/60 group-hover:text-primary-content">
              {{ index + 1 }}
            </span>
          </div>
    ...
    <main class="min-h-0 flex-1 rounded-2xl border border-base-300 bg-base-100 p-4">
      <slot
    ...
    >
      <div class="mb-3 flex items-center justify-between gap-3">
        <div>
          ...
          <p class="text-sm text-base-content/60">
            A living receipt for your choices, minus the part where a receipt accuses you of buying too many snacks.
          </p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        >
          <div class="h-20 w-20 shrink-0 overflow-hidden rounded-2xl bg-base-300">
            <img
              v-if="item.image"
              :src="item.image"
              :alt="item.label"
...
import type {
  ...
  BuilderSectionKey,
} from '@/stores/seeds/builderSections'
...
const activeSection = computed(() => {
  return props.sections[activeSectionIndex.value] ?? props.sections[0]
})

const canGoBack = computed(() => activeSectionIndex.value > 0)

const canGoNext = computed(() => {
  return activeSectionIndex.value < props.sections.length - 1
})

function setSection(sectionKey: BuilderSectionKey) {
  const exists = props.sections.some((section) => section.key === sectionKey)
...
function goBack() {
  ...
  const section = props.sections[activeSectionIndex.value - 1]
...
function displayValue(value: BuilderChoiceSummary['value']) {
  if (value === null || value === undefined || value === '') return 'Not selected yet'
