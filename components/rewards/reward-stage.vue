<!-- components/rewards/reward-stage.vue -->
<template>
  <div class="flex h-full flex-col">
    ...
    >
      <Icon name="kind-icon:gift" class="h-12 w-12 text-base-content/20" />
      <div>
    ...
    <template v-else>
      ...
      <div class="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
        ...
        <!-- Collection presets shown as chips when on collection card -->
        <div
          v-if="
            store.activeStep?.inputType === 'text' &&
            store.activeStep.choices?.length
          "
          class="flex flex-wrap gap-2"
        >
          <button
            v-for="choice in store.activeStep.choices"
            :key="choice.value"
            type="button"
            class="rounded-full border px-3 py-1 text-xs font-semibold transition-all"
            :class="
              stagedValue === choice.value
                ? 'border-primary bg-primary/15 text-primary'
                : 'border-base-300 bg-base-200 text-base-content/70 hover:border-primary/40 hover:text-primary'
            "
            @click="stagedValue = choice.value"
          >
            {{ choice.label }}
          </button>
        </div>
