...
function contextualFallback(
  ...
  const prompt = [
    ...
    'every surface bare and unmarked, one continuous scene',
  ].join(' ')
    ...
    res = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          `If people belong in the scene, include this clause verbatim: ${DEFAULT_CAST_ART_DIRECTION}.`,
          ...
          'Say what the scene DOES contain instead: "every surface bare and unmarked" rather than naming text, "one continuous scene" rather than naming a collage.',
          'Never write the word "frame". Krea paints it as a physical picture frame in every sense: say "build", "body" or "physique" for anatomy. Never write "picture" or "composition" either: Krea paints those as a framed picture on a wall. Leave the shape to the requested size, and keep "frame" for a real frame that is genuinely in the scene.',
