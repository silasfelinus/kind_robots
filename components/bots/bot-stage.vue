...
<template>
  <div class="flex h-full flex-col">
    ...
    >
      <Icon name="kind-icon:robot" class="h-12 w-12 text-base-content/20" />
      <div>
    ...
    <template v-else>
      ...
      <div class="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
        ...
        <!-- Preset choices (type card) -->
        <adventure-preset v-if="builder.activeStep?.inputType === 'preset'" />

        <!-- Personality multi-select -->
        <bot-personality
          v-else-if="builder.activeStep?.inputType === 'personality'"
        />

        <!-- Opening messages -->
        <bot-intros v-else-if="builder.activeStep?.inputType === 'intros'" />

        <!-- Modules -->
        <bot-modules v-else-if="builder.activeStep?.inputType === 'modules'" />
