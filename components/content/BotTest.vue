...
<template>
  ...
  >
    <p class="text-2xl">{{ activeBot.botType }}</p>
    ...
    <p>{{ activeBot.createdAt }}</p>
    <p>{{ activeBot.updatedAt }}</p>
  </div>
  <div v-if="activeBot" class="col-span-full sm:col-span-1">
    ...
    <div v-if="activeBot" class="col-span-full sm:col-span-1">
      ...
      <div v-if="conversations[activeBot.id]" class="p-4 mt-4 rounded bg-primary text-white">
        <!-- response section -->
        <div
          v-for="conversation in conversations[activeBot.id]"
          :key="conversation.id"
          class="mt-4"
        >
          <!-- chat bubble -->
...
let conversations = ref<Record<string, Array<{ id: number; message: string }>>>({})

// computed value for bot avatar
let botAvatar = computed(() => activeBot.value.avatarImage || '/images/amibot1.png')
...
const submit = async () => {
  ...
  try {
    ...
    if (response.status === 200) {
      const botConversations = conversations.value[activeBot.value.id] || []
      botConversations.push({ id: Date.now(), message: response.data.message })
      prompt.value = ''
      conversations.value = { ...conversations.value, [activeBot.value.id]: botConversations }
    } else {
