...
<!--
  ...
  Split out of reward-interact, which had grown to 1097 lines -- 206 of
  template and 890 of script -- because a whole encounter engine lived in the
  ...
  do with them uniquely." So nothing about HOW a Reward encounter works changed
...
/*
 * Session membership lives in chatStore, keyed by scope, because the ROUTER
 * needs it too: reward-interact stays in interact mode while a conversation is
 * in progress, and a private ref here would have hidden that from it.
 */
const SESSION_SCOPE = 'reward'
...
const sessionChats = computed<Chat[]>(() => {
  return chatStore.chats.filter((chat) =>
    chatStore.sessionChatIds(SESSION_SCOPE).includes(chat.id),
  )
})
...
async function createRewardChat(content: string) {
  ...
  chatStore.addSessionChat(SESSION_SCOPE, newChat.id)
