#!/usr/bin/env python3
"""Find or migrate hand-rolled `loading loading-dots loading-sm` busy
indicators to `kr-spinner-sm-dots`.
...
the second-smallest of the four sibling sizes: 6 exact-match occurrences
across 6 files (model-builder-recipe-selector.vue, model-builder-item-panel.vue,
conductor/storybook-page.vue, art/generate-button.vue, art/art-generator.vue,
pages/taskmaster-page.vue). Siblings `-xs` (8/3), `-md` (3/3), and `-lg`
(2/2) are migrated by their own sibling codemods in the same slice.

Named `kr-spinner-sm-dots` (size before animation-variant) rather than
`kr-spinner-dots-sm` to keep every `kr-spinner-<size>*` name alphabetically
...
`loading-sm`) are touched, and only in static `class="..."` attributes --
never `:class`/`v-bind:class` bindings (see _class_attr.py). A source that
already carries `kr-spinner-sm-dots`, or is missing any one of the three
...
PRIMITIVE = "kr-spinner-sm-dots"
BASE_TOKENS = {"loading", "loading-dots", "loading-sm"}
