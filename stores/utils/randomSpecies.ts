...
export function useRandomSpecies() {
  ...
  function generateHybridSpecies() {
    ...
    const base = [...fantasySpecies, ...sciFiSpecies, ...cartoonSpecies][
      Math.floor(
        Math.random() *
          (fantasySpecies.length + sciFiSpecies.length + cartoonSpecies.length),
      )
    ]
    const hybridTypes = [
      'kin',
  ...
  const speciesList = [
    ...
    ...Array(100)
      .fill(0)
      .map(() => randomAnimal()), // Add 100 random animals
    ...Array(100)
      .fill(0)
      .map(() => generateHybridSpecies()), // Add 100 hybrids
  ]
