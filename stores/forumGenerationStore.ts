...
function defaultPrompt(post: ForumPost): string {
  const source = [post.title?.trim(), post.content.trim()].filter(Boolean).join('\n\n')
...
export const useForumGenerationStore = defineStore('forumGenerationStore', () => {
  ...
  const attachedArt = computed(() =>
    sourcePost.value?.attachments.find((entry) => entry.kind === 'ART_IMAGE') ?? null,
  )
  ...
  async function queueArt(): Promise<boolean> {
    ...
    try {
      ...
      message.value =
        `Queued ArtJob #${response.data.jobId}. Charged ${response.data.mana.charged} generation units; this compute spend is not a charitable donation.`
