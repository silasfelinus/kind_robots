...
<template>
  <section
    ...
    @dragover.prevent
    @drop="onDrop"
    @click.self="codeCardStore.selectNode(null)"
  >
    <div
      class="relative"
      :style="{
        width: `${codeCardStore.canvasBounds.width}px`,
        height: `${codeCardStore.canvasBounds.height}px`,
      }"
    >
      ...
      <svg
        ...
        :width="codeCardStore.canvasBounds.width"
        :height="codeCardStore.canvasBounds.height"
      >
        <path
      ...
      <code-card-node
        v-for="node in codeCardStore.nodes"
        :key="node.id"
        :node="node"
        class="absolute z-20"
        :style="{
      ...
      <div
        v-if="!codeCardStore.nodes.length"
...
import { useCodeStore, type CodeCardKind } from '@/stores/codeStore'
...
const getPortIndex = (nodeId: string, portId: string, direction: 'input' | 'output') => {
  const node = codeCardStore.nodes.find((candidate) => candidate.id === nodeId)
  const definition = node ? codeCardStore.getDefinition(node.kind) : null
...
const visibleConnections = computed(() => {
  return codeCardStore.connections
    .map((connection) => {
...
const onDrop = (event: DragEvent) => {
  const kind = event.dataTransfer?.getData('kindrobots/code-card-kind') as CodeCardKind
  ...
  codeCardStore.addNode(kind, x, y)
}
</script>