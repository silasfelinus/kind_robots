...
export async function getChatGptAsset(
  rawInput: Record<string, unknown>,
  headers: ChatGptActionHeaders,
): Promise<{
  ok: true
  ...
  const idValue = rawInput.id
  const id = typeof idValue === 'number' ? idValue : Number(idValue)

  if (!Number.isFinite(id) || id <= 0) {
    fail('A valid ArtImage id is required', 400)
  }

  const asDataUrl = asBoolean(rawInput.asDataUrl, false)
  ...
  const isOwner = artImage.userId === user.id
  const isAdminUser =
  ...
  const fileType = artImage.fileType || 'image/png'
  const imageData = artImage.imageData || ''

  return {
    ok: true,
    data: {
      artImageId: artImage.id,
      artId: artImage.artId ?? null,
      fileName: artImage.fileName ?? null,
      fileType,
      ...(asDataUrl
        ? {
            dataUrl: imageData.startsWith('data:')
              ? imageData
