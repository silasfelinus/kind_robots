<template>
  <div class="p-6 max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex gap-4">
      <label
        ><input type="radio" value="text" v-model="inputType" /> Text</label
      >
      <label
        ><input type="radio" value="image" v-model="inputType" /> Image</label
      >
    </div>
    ...
    <div v-else>
      ...
      <img
        v-if="imageData"
        :src="imageData"
        class="mt-2 max-w-sm border rounded"
      />
    </div>
    ...
    <div class="mt-4 space-y-4">
      ...
      <div
        v-for="(step, index) in modifierSteps"
        :key="step.id"
        class="p-4 border rounded bg-base-100 space-y-2"
      >
        <div class="flex justify-between items-center">
          ...
          <div class="flex gap-2">
            ...
            <button class="btn btn-xs btn-error" @click="removeStep(step.id)">
              ✕
            </button>
          </div>
        </div>
...
import { useComfyStore, type ModifierType } from '@/stores/comfyStore'
...
const newStepType = ref<ModifierType | ''>('')
