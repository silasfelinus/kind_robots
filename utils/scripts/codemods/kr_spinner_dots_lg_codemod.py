#!/usr/bin/env python3
"""Find or migrate hand-rolled `loading loading-dots loading-lg` busy
indicators to `kr-spinner-lg-dots`.
...
the largest of the four sibling sizes: 2 exact-match occurrences across 2
files (storybook/storybook-reading.vue, coloring/coloring-book-studio.vue).
Siblings `-xs` (8/3), `-sm` (6/6), and `-md` (3/3) are migrated by their own
sibling codemods in the same slice.

Named `kr-spinner-lg-dots` (size before animation-variant) rather than
`kr-spinner-dots-lg` to keep every `kr-spinner-<size>*` name alphabetically
...
`loading-lg`) are touched, and only in static `class="..."` attributes --
never `:class`/`v-bind:class` bindings (see _class_attr.py). A source that
already carries `kr-spinner-lg-dots`, or is missing any one of the three
...
PRIMITIVE = "kr-spinner-lg-dots"
BASE_TOKENS = {"loading", "loading-dots", "loading-lg"}
