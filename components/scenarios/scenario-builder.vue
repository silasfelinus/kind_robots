  ...
  <div class="relative flex h-full w-full flex-col overflow-hidden">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <main class="flex flex-1 flex-col overflow-hidden">
        <scenario-stage class="flex-1 overflow-y-auto" />
      </main>
    </div>
...
const canSave = computed(
  () =>
    Boolean(scenarioStore.scenarioForm.title?.trim()) &&
    Boolean(scenarioStore.scenarioForm.description?.trim()) &&
    Boolean(
      Array.isArray(scenarioStore.scenarioForm.intros)
        ? (scenarioStore.scenarioForm.intros as string[]).some((s) => s.trim())
...
async function doSave() {
  feedback.value = ''
  const result = await builder.saveScenario()
  if (result?.id) {
    feedback.value = 'Scenario saved.'
    isError.value = false
  } else {
    feedback.value = builder.lastError ?? 'Save failed.'
    isError.value = true
  }
  setTimeout(() => {
    feedback.value = ''
