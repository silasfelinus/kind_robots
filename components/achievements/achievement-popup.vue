<!-- /components/content/achievements/achievement-popup.vue -->
<template>
  <div
    v-if="achievement && !userStore.isGuest"
    class="pointer-events-auto fixed inset-0 z-70 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm"
    aria-labelledby="achievement-popup"
    role="dialog"
    aria-modal="true"
  >
    <div
...
const achievement = computed<AchievementLike | null>(() => {
  const m = achievementStore.unconfirmedAchievements?.[0]
  return (m ?? null) as AchievementLike | null
})

watch(achievement, async (newAchievement, oldAchievement) => {
  if (newAchievement && !oldAchievement && !userStore.isGuest) {
    await nextTick()
    confetti({ particleCount: 150, spread: 90, origin: { y: 0.6 } })
    console.log(
      `[achievement-popup] Confetti triggered for ${newAchievement.label}`,
    )
  }
})

const acknowledgeAchievement = async (): Promise<void> => {
  const current = achievement.value
  if (!current) {
    console.warn(
      '[achievement-popup] Tried to acknowledge, but no achievement found',
    )
    return
  }
  console.log(
    `[achievement-popup] Acknowledging achievement: ${current.label} (id: ${current.id})`,
  )
  await achievementStore.confirmAchievement(current.id)
}
</script>
