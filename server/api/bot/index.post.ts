...
export default defineEventHandler(async (event) => {
  try {
    const botData = await readBody(event) // Expecting a single bot's data
    ...
    return { success: true, bot: result.bot } // Return the created bot
  } catch (error) {
...
export async function addSingleBot(
  ...
  try {
    ...
    return { bot, error: null } // Return the created bot
  } catch (error: unknown) {
    const errorMessage =
