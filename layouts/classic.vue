<template>
  <div class="flex flex-col h-screen overflow-hidden p-2 bg-base-200">
    ...
    <main class="h-[calc(100vh-2rem-32px)] overflow-hidden flex items-center">
      <!-- Slot -->
      <div
        class="flex-grow flex flex-col items-center h-[calc(100%-16px)] max-h-[calc(100vh-2rem-32px-16px)] overflow-y-auto p-2 m-2 rounded-2xl border bg-primary"
      >
        <slot />
      </div>
      ...
      <div class="flex flex-col w-1/5 h-full bg-base-200">
        ...
        <div
          class="flex-3 flex items-start justify-start rounded-2xl p-2 m-2 border bg-primary overflow-y-auto"
