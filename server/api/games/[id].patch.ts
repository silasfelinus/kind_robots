...
async function handleJoinGame(
  gameId: number,
  playerName: string,
): Promise<ExtendedErrorHandlerOutput> {
  try {
...
async function handleUpdatePlayerPoints(
  playerId: number,
  points: number,
): Promise<ExtendedErrorHandlerOutput> {
  try {
...
async function handleLeaveGame(
  ...
  try {
    ...
    return {
      success: true,
      message: 'Player status updated to LEFT',
...
async function handleGameUpdates(
  gameId: number,
    ...
    switch (body.action) {
      ...
      case 'updatePoints': {
        const player = await prisma.player.findFirst({
          where: { name: body.playerName, gameId: gameId },
        })
        if (!player) {
          return {
            success: false,
      ...
      case 'leave': {
        const playerToLeave = await prisma.player.findFirst({
          where: { name: body.playerName, gameId: gameId },
        })
        if (!playerToLeave) {
          return {
      ...
      default: {
        const updateData: UpdateData = {
          ...
          isFinished:
            body.isFinished !== undefined ? body.isFinished : undefined,
        }
