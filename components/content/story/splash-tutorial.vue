  ...
  <div class="relative w-full h-full overflow-hidden rounded-2xl z-40">
    ...
    <div class="relative z-20 container px-4 py-6 space-y-8 max-w-4xl mx-auto">
      ...
      <div class="relative space-y-2 text-center">
        ...
        <h1 v-if="title" class="text-white text-3xl md:text-5xl font-bold">
          The {{ title }} Room
        </h1>

        <h2
          v-if="description"
          class="text-white text-base md:text-lg lg:text-xl font-medium"
        >
          {{ description }}
        </h2>
      </div>
      ...
      <div @click.stop>
        <mode-row class="w-full min-h-10 bg-primary pointer-events-auto" />
      </div>
